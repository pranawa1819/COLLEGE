//1.	WAP to simulate a Turing Machine that accepts language L= {WW^r|n>=1} over alphabet {a,b} using c++.
...
int main() {
    ...
    while (oldtapehead != tapehead) {
        ...
        if (state == 0) {
            if (action('a', 'B', 'R', 1) || action('b', 'B', 'R', 4) || action('B', 'B', 'R', 6)) {
                continue;
            }
        } else if (state == 1) {
            if (action('a', 'a', 'R', 1) || action('b', 'b', 'R', 1) || action('B', 'B', 'L', 2)) {
                continue;
            }
        } else if (state == 2) {
            if (action('a', 'B', 'L', 3)) {
            ...
            if (action('a', 'a', 'L', 3) || action('b', 'b', 'L', 3) || action('B', 'B', 'R', 0)) {
                continue;
            }
        } else if (state == 4) {
            if (action('a', 'a', 'R', 4) || action('b', 'b', 'R', 4) || action('B', 'B', 'L', 5)) {
                continue;
            }
        } else if (state == 5) {
            if (action('b', 'B', 'L', 3)) {
